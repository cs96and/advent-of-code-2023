...
class Cell
	...
	def get_neighbours(max_y, max_x)
		...
		neighbours.reject do |n|
			(@parent && n.coord == @parent.coord) || n.y < 0 || n.y >= max_y || n.x < 0 || n.x >= max_x || n.same_three_times
		end
	end

	def same_three_times
...
def a_star(grid, start, goal)
	...
	until open_queue.empty?
		...
		neighbours.each do |n|
			...
			f = g + n.distance(goal)
			# Need to keep track of the f score of this node with its 3 parents
			f_key = [n, cell, cell.parent, cell.parent&.parent]
			old_f = f_score[f_key]
			if f < old_f
